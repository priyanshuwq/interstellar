...
function useInViewNative(ref: React.RefObject<Element | null>, margin = "-20% 0px -20% 0px") {
...
export default function MissionSection() {
  ...
  const isInView = useInViewNative(sectionRef, "-20% 0px -20% 0px");
  const quality = useDevicePerformance();

  const dpr: [number, number] =
    quality === "high" ? [1, 1.5] : [1, 1];

  const objectives = [
    {
      id: "01",
      title: "Find a new home",
      desc: "Earth's atmosphere is failing. We must locate a habitable world across the galaxy.",
    },
    {
      id: "02",
      title: "Solve the equation",
      desc: "Professor Brand's gravity equation requires quantum data from inside a singularity.",
    },
    {
      id: "03",
      title: "Save humanity",
      desc: "Plan A: Evacuate Earth. Plan B: Population bomb on a new world.",
    },
  ];
  ...
  return (
    <section
      ref={sectionRef}
      id="mission"
      className="min-h-screen py-24 px-6 md:px-20 max-w-7xl mx-auto flex flex-col justify-center relative z-10"
    >
      ...
      <div className="relative z-10 w-full md:w-2/3">
        ...
        <h3 className="font-display text-3xl md:text-5xl font-medium tracking-wide mb-16 text-foreground max-w-3xl drop-shadow-lg">
          Mankind was born on Earth. It was never meant to{" "}
          <span className="font-serif italic text-amber">die</span> here.
        </h3>
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {objectives.map((obj) => (
            <div
              key={obj.id}
              className="p-6 md:p-8 border-t border-l border-white/10 bg-black/40 hover:bg-black/60 transition-all duration-500 group relative overflow-hidden backdrop-blur-md hover:border-amber/40"
            >
              ...
              <div className="flex items-center gap-3 mb-6 relative z-10">
                <span className="font-mono text-xs text-amber/80 tracking-[0.2em]">
                  {obj.id}
                </span>
                <div className="h-[1px] w-8 bg-white/20 group-hover:w-16 group-hover:bg-amber/50 transition-all duration-500"></div>
              </div>
              <h4 className="font-display text-2xl mb-4 text-white group-hover:text-amber/90 drop-shadow-md transition-colors duration-300 tracking-wider relative z-10 uppercase">
                {obj.title}
              </h4>
